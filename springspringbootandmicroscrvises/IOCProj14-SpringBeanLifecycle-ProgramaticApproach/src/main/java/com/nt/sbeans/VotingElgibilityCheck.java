...
public class VotingElgibilityCheck implements InitializingBean,DisposableBean{
	...
	public void afterPropertiesSet() throws Exception{
		...
		verifiedOn=new Date();
		//check wheather important properties are cfg with current values or not
		if(name==null || name.trim().length()==0 || age<0) 
			throw new IllegalArgumentException("Invalid values aer injected for sprint bean propeties(name,age)");
